...
def run():
    ...
    logger.info("热点新闻耗时: " + str((t2 - t1).seconds) + "s")
    logger.info("今日要闻耗时: " + str((t3 - t2).seconds) + "s")
    logger.info("热门资讯耗时: " + str((t4 - t3).seconds) + "s")
    logger.info("本次抓取总耗时: " + str((t4 - t1).seconds) + "s")
    # 仅两天的信息，就有三千多条...我的天
